...
// Priority LoRA fields for Replicate models (mirrors public/index.html Fill targets).
function findLoraField(schema) {
  const params = schema?.properties || schema?.params;
  if (!params) return null;
  for (const k of ['loras', 'lora_url', 'lora_list', 'lora_weights', 'extra_lora', 'extra_lora_weights', 'lora_scale']) {
...
export default function LoraPicker({ variant, schema, model, modelId, params, onParams, notify }) {
  ...
  const fill = (l, useFile) => {
    ...
    const spec = (schema?.properties || schema?.params || {})[field];
    if (spec?.type === 'array') {
