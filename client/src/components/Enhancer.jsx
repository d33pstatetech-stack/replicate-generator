...
// Props: model, params, schema, onUse(text), notify, onEnhancement(id).
export default function Enhancer({ model, params, schema, onUse, notify, onEnhancement }) {
  ...
  const ctx = getEnhancerContext(model, params, schema || {});
...
function pushSaved(entry) {
  try {
    const saved = JSON.parse(localStorage.getItem('replicate_saved') || '[]');
    saved.unshift(entry);
    localStorage.setItem('replicate_saved', JSON.stringify(saved.slice(0, 50)));
  } catch {
    /* storage blocked */
  }
}
