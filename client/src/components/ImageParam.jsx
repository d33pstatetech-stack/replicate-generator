...
// Image / image-array param: local file (→ data URI, mirroring public/index.html
// fileToDataUri — there is NO /api/upload on this backend) + R2 file + URL paste.
...
export default function ImageParam({ name, multi, value, onChange, notify }) {
  ...
  const put = async (file) => {
    ...
    if (!file.type.startsWith('image/')) {
      notify && notify('Only image files are accepted here', 'error');
      return;
    }
    setBusy(true);
    try {
      ...
      onChange(multi ? [...list, uri].slice(0, 9) : uri);
    } catch (e) {
      notify && notify(`Read failed: ${e.message}`, 'error');
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="space-y-2">
      ...
      >
        ...
        <p className="text-xs text-gray-400 mt-1">{busy ? 'Reading…' : multi ? 'Add images (up to 9, sent as data URIs)' : 'Tap or drop an image (sent as data URI)'}</p>
